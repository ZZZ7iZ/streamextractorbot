...
async def extract_audio(client, message, data):
    await message.edit_text("جارٍ استخراج الصوت من الملف...")
    ...
    if data['name'] == "mp3":
        ...
        if rcode != 0:
            await message.edit_text("**حدث خطأ. راجع المطور لمزيد من المعلومات.**")
            print(err)
...
async def extract_subtitle(client, message, data):
    await message.edit_text("استخراج الصوت من الملف")
